classdef Contacts < handle
    %CONTACTS
    % The Contact class handles the computation of the contact forces and the impact,

    % Contact Properties:
    %     num_vertices - the number of vertices per foot
    %     foot_print - this vector says if the vertex was in contact (1) or not (0)
    %     was_in_contact - this vector says if the vertex is in contact (1) or not (0)
    %     is_in_contact - friction coefficient
    %     S - selector matrix for the robot torque
    %     mu - friction coefficient
    %     A, b, Aeq, beq - matrix used in the optimization problem
    %
    % Contact Methods:
    %     compute_contact - computes the wrench and the state velocity after an impact
    ...
    methods
        ...
        function obj = Contacts(foot_print, robot, friction_coefficient)
            %CONTACTS The Contact class needs the coordinates of the ver
        ...
        function [generalized_total_wrench, wrench_left_foot, wrench_right_foot, base_pose_dot, s_dot] = ...
            ...
            % OUTPUTS
            % generalized_total_wrench - the sum of the generalized_ext_wrench and the generalized contact wrench
    ...
    methods (Access = private)
        ...
        function [J_feet, JDot_nu_feet] = compute_J_and_JDot_nu(obj, robot)
            % returns the Jacobian and J_dot_nu relative to the vertices (Not the sole frames!)
        ...
        function contact_points = compute_contact_points(obj, robot)
            % returns the vertical coordinate of every vertex
        ...
        function [base_pose_dot, s_dot] = compute_velocity(obj, M, J_feet, robot, base_pose_dot, s_dot)
            % returns the configuration velocity
        ...
        function free_contact_acceleration = compute_free_contact_acceleration(obj, J_feet, free_acceleration, JDot_nu_feet)
            % returns the acceleration of the feet with NO contact forces

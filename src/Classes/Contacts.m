classdef Contacts < handle
    ...
    properties
        num_vertices = 4;
        foot_print;
        is_in_contact = ones(8, 1); % matrix used for the check of the contact
        S; % selector matrix
    ...
    methods
        ...
        function [generalized_total_wrench, wrench_left_foot, wrench_right_foot, base_pose_dot, s_dot] = ...
                compute_contact_forces(obj, robot, torque, generalized_ext_wrench, base_pose_dot, s_dot)
            ...
            for ii = 1:4
                ...
                now_in_contact(ii) = left_z_foot_print(ii) <= 0;
                now_in_contact(ii + 4) = right_z_foot_print(ii) <= 0;
            end
            ...
            [wrench_left_foot, wrench_right_foot] = obj.compute_contact_wrench_in_sole_frames(contact_forces, H_LFOOT, H_RFOOT, obj.foot_print);

            for ii = 1:8

                if now_in_contact(ii) == 1 && obj.is_in_contact(ii) == 0
                    N = (eye(robot.NDOF + 6) - M \ (J_feet' * ((J_feet * (M \ J_feet')) \ J_feet)));
            ...
            obj.is_in_contact = now_in_contact;
        end
        ...
        function forces = compute_unilateral_linear_contact(obj, J_feet, M, h, torque, JDot_nu_feet, contact_point, generalized_ext_wrench)
            ...
            %             forces = quadprog(H, f, obj.A, obj.b, obj.Aeq, obj.beq, [], [], [], options);
            %             forces = qpOASES(H, f, obj.A, [], [], [], obj.b);
        end

        function [wrench_left_foot, wrench_right_foot] = compute_contact_wrench_in_sole_frames(obj, contact_forces, H_LFOOT, H_RFOOT, foot_print)
            ...
            for i = 1:4
                ...
                wrench_right_foot(4:6) = wrench_right_foot(4:6) - wbc.skew(foot_print(:, i)) * (R_RFOOT' * contact_forces_right(j:j + 2));
            end
